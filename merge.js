...
var ang, c, canvas, combinations, cx, cy, divideQuadrants, drawgrid, e, getPixel, h, height, i, j, parts, pixels, rand, rng_state, s, seed, smallBoundingBox, w, weightMerger, width, x, y, _i, _j, _k, _len, _ref, _ref1;
...
smallBoundingBox = function(x, y) {
  var sched, xmax, xmin, ymax, ymin, _k, _len, _ref1;
  xmin = 5;
  ymin = 5;
  xmax = -1;
  ymax = -1;
  sched = [[0, 0], [3, 3], [0, 1], [3, 2], [1, 0], [2, 3], [0, 2], [3, 1], [2, 0], [1, 3], [0, 3], [3, 0], [1, 1], [2, 2], [1, 2], [2, 1]];
  for (_k = 0, _len = sched.length; _k < _len; _k++) {
    _ref1 = sched[_k], i = _ref1[0], j = _ref1[1];
...
weightMerger = function(_arg, _arg1) {
  ...
  if (dist < 20 && asum / amax > 0.8) {
    return [waste, bound];
  }
  return null;
};

divideQuadrants = function(x, y, size) {
  var a, b, bound, box, boxes, boxtmp, e, h, h1, h2, half, mx, my, pair, pairs, score, skipbox, w1, w2, waste1, waste2, weight, x1, x2, y1, y2, _k, _l, _len, _len1, _len2, _m, _ref1, _ref2;
  if (size === 4) {
  ...
  mx = x + half;
  my = y + half;
  boxes = divideQuadrants(x, y, half).concat(divideQuadrants(mx, y, half), divideQuadrants(mx, my, half), divideQuadrants(x, my, half));
  skipbox = [];
  bound = 32;
  if (size > bound * 4) {
    boxtmp = boxes;
    ...
    for (_k = 0, _len = boxtmp.length; _k < _len; _k++) {
      ...
      if (Math.abs(x1 - mx) < bound || Math.abs(x1 + w1 - mx) < bound || Math.abs(y1 - my) < bound || Math.abs(y1 + h1 - my) < bound) {
        boxes.push(box);
      } else {
        skipbox.push(box);
      }
  ...
  while (boxes.length > 1) {
    ...
    _ref1 = pairs[0], score = _ref1[0], bound = _ref1[1], a = _ref1[2], b = _ref1[3];
    boxes = (function() {
      var _l, _len1, _results;
      _results = [];
  ...
  for (_m = 0, _len2 = boxes.length; _m < _len2; _m++) {
    _ref2 = boxes[_m], x = _ref2[0], y = _ref2[1], w = _ref2[2], h = _ref2[3], e = _ref2[4];
